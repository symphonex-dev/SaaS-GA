...
- **Vérification** : tests de garde-fous IA verts (aucune sortie hors du schéma Zod n'atteint l'utilisateur) ; relecture manuelle du prompt système.
  ...
  - **Reste à faire** : brancher un vrai provider (`AI_PROVIDER=openai` + `AI_API_KEY`) et vérifier le format de sortie réel — les tests n'exercent que le mock ; alimenter la base d'offres de production offre par offre, selon la procédure de `ACTIONS_MANUELLES.md` §7 bis (le seed ne contient que des offres de démonstration, dont les dates de vérification sont figées début 2026 et donc déjà périmées) ; exécuter la migration `20260829130000_comparison_offer_audit` sur une base PostgreSQL réelle.
...
### 10.17 Base d'offres de production

- **La base de production n'a pas de seed.** Le seed de démonstration est refusé en production (§10.1) et ses offres sont de toute façon échues. Les vraies offres passent par les routes d'administration, une par une ; la procédure d'exploitation est dans `ACTIONS_MANUELLES.md` §7 bis.
- **Le total « économies potentielles » du tableau de bord ne compte plus que les offres `FRESH`.** Il ne filtrait que sur `nextCheckAt` : une offre vérifiée il y a plus de 30 jours, mais pas encore échue, y entrait, alors que le comparateur la marque obsolète et non recommandable (A.6, §10.7). Le tableau de bord et `totalPotentialSavings` appliquent désormais la même règle de fraîcheur.
- **Une date de vérification ne peut pas être dans le futur.** Datée du futur, une offre avait un âge négatif et restait `FRESH` jusqu'à sa prochaine vérification sans avoir été vérifiée. Le contrôle est fait par le service (tolérance d'horloge de 5 minutes), pas par le schéma partagé : il dépend de l'heure, et un schéma Zod doit rester une fonction pure de son entrée.
- **L'offre et sa trace d'audit sont écrites dans une même transaction.** Le code l'affirmait déjà, mais écrivait la trace par un second appel : un échec entre les deux laissait une modification sans trace (A.8).
- **Limite connue : un seul nom par offre.** Une offre n'est recommandée que si `serviceName` se réduit exactement au nom de commerçant normalisé (« Netflix ») ; « Netflix Standard » reste une suggestion. Le nom de la formule doit donc aller dans `featuresIncluded`, qui s'affiche tel quel, sans traduction. Distinguer proprement le nom du service et celui de la formule demanderait un champ de plus au schéma — non fait.

### 10.18 Planification du job d'expiration

- **Le job est déclenché par GitHub Actions, pas par Vercel Cron** (`.github/workflows/expire-overdue.yml`, tous les jours à 03:17 UTC). Vercel Cron appelle la route en `GET`, alors que `POST /api/billing/expire-overdue` n'accepte que `POST` : le planificateur de la plateforme aurait reçu un 405 sans que personne ne le voie. Le workflow appelle la route telle qu'elle est, sans modification du code.
- **Le secret est partagé entre Vercel et le dépôt** (`BILLING_CRON_SECRET`, secret GitHub de même nom) ; l'URL de l'API est une variable du dépôt (`API_BASE_URL`). Un secret ou une URL manquants font échouer le workflow explicitement, tout comme une réponse différente de 200 : un job qui échoue en silence serait pire qu'aucun job.
- **Un retard d'exécution ne donne aucun accès indu** : `effectivePlan()` applique déjà la fin de période à la lecture (§10.8). Le job ne fait que remettre la base en cohérence, et il est idempotent — les nouvelles tentatives de `curl` sont donc sans risque.
